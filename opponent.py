...
class Tooth(pygame.sprite.Sprite):
    def __init__(self,pos, frames, groups, collision_sprites):
        ...
        self.hit_ticker = Ticker(250)

    def reverse(self):
         if not self.hit_ticker.active:
          self.direction *= -1
          self.hit_ticker.activate()

    def update(self,dt):

        self.hit_ticker.update()
...
class Shell(pygame.sprite.Sprite):
     ...
     def state_management(self):
          ...
          if player_near and player_front and player_level and not self.shoot_ticker.active:
               self.state = 'fire'
               self.frame_index = 0
               self.shoot_ticker.activate()


     def update(self,dt):
          self.shoot_ticker.update()
...
class Pearl(pygame.sprite.Sprite):
     def __init__(self, pos, groups, surf, direction, speed):
          ...
          self.speed = speed
          self.z = Z_LAYERS['main']  
          self.tickers = {'lifetime' : Ticker(5000), 'reverse': Ticker(250)}   
          self.tickers['lifetime'].activate()

     def reverse(self):
         if not self.tickers['reverse'].active:
              self.direction *= -1
              self.tickers['reverse'].activate()

     def update(self,dt):
          for ticker in self.tickers.values():
               ticker.update()
          self.rect.x += self.direction * self.speed * dt 
          if not self.tickers['lifetime'].active:
               self.kill()